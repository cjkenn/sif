...
pub struct VM {
    ...
    /// TODO: we can make a "function declaration" section to avoid this. Or we could do
    /// a separate pass to do this, like the jumptab population. However, we'd need to
    /// make changes to the parser as well, as we cannot call a function before it's declared
    /// while parsing anyway.
...
impl VM {
    ...
    fn execute(&mut self) -> Result<(), RuntimeErr> {
        ...
        match curr {
            ...
            Op::Fn { name, params: _ } => {
                self.fntab.insert(name.clone(), self.cdr);
            }
            Op::Nop => {}
            Op::Stop => {
